...
import com.example.rumens.showtime.inject.component.DaggerSpecialComponent;
...
public class SpecialActivity extends BaseSwipeBackActivity<IBasePresenter> implements ISpecialView {
    ...
    private int[] mSkipId=new int[20];
    private String mSpecialId;

    public static void launch(Context mContext, String specialID) {
        Intent intent = new Intent(mContext, SpecialActivity.class);
        intent.putExtra(SPECIAL_KEY, specialID);
    ...
    private void handleTagLayout(List<SpecialItem> specialItems) {
    ...
    private String clipHeadStr(String header) {
        String head = null;
        int index = header.indexOf(" ");
        if (index != -1) {
            head = header.substring(index, header.length());
        }
        return head;
    }
