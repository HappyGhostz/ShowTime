...
public class BookSettingManager {
    private volatile static BookSettingManager manager;

    public static BookSettingManager getInstance() {
        return manager != null ? manager : (manager = new BookSettingManager());
    }
    ...
    public int getReadBrightness() {
        return SharedPreferencesUtil.getInstance().getInt(getLightnessKey(),
                (int) ScreenUtils.getScreenBrightness(App.getContext()));
    }
