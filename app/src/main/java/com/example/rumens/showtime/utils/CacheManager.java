...
import com.example.rumens.showtime.api.bean.BookLists;
...
public class CacheManager {
    ...
    public List<BookLists.BookListsBean> getCollectionList() {
        List<BookLists.BookListsBean> list = (ArrayList<BookLists.BookListsBean>) ACache.get(
                App.getContext()).getAsObject(getCollectionKey());
        ...
        for (BookLists.BookListsBean bean : list) {
            if (bean != null) {
                if (TextUtils.equals(bean._id, bookListId)) {
                    list.remove(bean);
                    ACache.get(App.getAppContext()).put(getCollectionKey(), (Serializable) list);
                    break;
                }
            }
        }
     ...
     * @return
     */
    public List<BookMixATocBean.mixToc.Chapters> getTocList(Context mContext, String bookId) {
        Object obj = ACache.get(mContext).getAsObject(getTocListKey(bookId));
        if (obj != null) {
            try {
                BookMixATocBean data = (BookMixATocBean) obj;
                List<BookMixATocBean.mixToc.Chapters> list = data.mixToc.chapters;
    ...
    public void saveTocList(Context mContext, String bookId, BookMixATocBean data) {
    ...
    public void saveChapterFile(String bookId, int chapter, ChapterReadBean.Chapter data) {
        File file = FileUtils.getChapterFile(bookId, chapter);
    ...
    public synchronized String getCacheSize() {
        ...
        try {
            ...
            if (FileUtils.isSdCardAvailable()) {
                String extCacheDir = App.getAppContext().getExternalCacheDir().getPath();
    ...
    public synchronized void clearCache(boolean clearReadPos, boolean clearCollect) {
        try {
            // 删除内存缓存
            String cacheDir = App.getAppContext().getCacheDir().getPath();
